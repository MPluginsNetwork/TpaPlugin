...
public class TpaManager {
    ...
    public void sendRequest(Player from, Player to) {
        if (from.getUniqueId().equals(to.getUniqueId())) {
            from.sendMessage("§cYou cant send tpa-requests to yourself!");
            return;
        }

        requests.put(to.getUniqueId(), new TpaRequest(from, to));
        from.sendMessage("§aTpa-Request has been send to " + to.getName());
    ...
    public void acceptRequest(Player player) {
        ...
        if (request == null) {
            player.sendMessage("§cYou dont have any tpa-requests! :(");
            return;
        }
